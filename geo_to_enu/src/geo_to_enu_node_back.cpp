...
Eigen::Matrix4d transform_enu_to_baselink = Eigen::Matrix4d::Identity();
Eigen::Quaterniond rtk_to_enu_q;
...
void heading_callback(const geometry_msgs::QuaternionStamped::ConstPtr& heading_msg)
{
    ...
    if (first_time)
    {
        Eigen::Quaterniond enu_q(heading_msg->quaternion.w, heading_msg->quaternion.x, heading_msg->quaternion.y, heading_msg->quaternion.z);
        transform_enu_to_baselink.block<3, 3>(0, 0) = enu_q.toRotationMatrix().inverse();
        first_time = false;
    }

    rtk_to_enu_q.w() = odom.pose.pose.orientation.w;
    rtk_to_enu_q.x() = odom.pose.pose.orientation.x;
    rtk_to_enu_q.y() = odom.pose.pose.orientation.y;
    rtk_to_enu_q.z() = odom.pose.pose.orientation.z;
}
...
void gps_to_xyz(const sensor_msgs::NavSatFix::ConstPtr& gps_msg)
{
    ...
    else
    {
        ...
        if (odom_pub.getNumSubscribers() > 0)
        {
            ...
            Eigen::Matrix4d rtk_to_enu_transform = Eigen::Matrix4d::Identity();
            rtk_to_enu_transform.block<3,3>(0,0) = rtk_to_enu_q.toRotationMatrix();
            Eigen::Vector3d rtk_to_enu_translation;
            rtk_to_enu_translation << odom.pose.pose.position.x, odom.pose.pose.position.y, odom.pose.pose.position.z;
            rtk_to_enu_transform.block<3, 1>(0, 3) = rtk_to_enu_translation;

            Eigen::Matrix4d rtk_to_baselink = Eigen::Matrix4d::Identity();
            rtk_to_baselink = transform_enu_to_baselink * rtk_to_enu_transform;

            Eigen::Vector3d rtk_to_baselink_translation = rtk_to_baselink.block<3, 1>(0,3);
      
            Eigen::Quaterniond rtk_to_baselink_q(rtk_to_baselink.block<3, 3>(0, 0));

            odom_last.pose.pose.position.x = rtk_to_baselink_translation.x();
            odom_last.pose.pose.position.y = rtk_to_baselink_translation.y();
            odom_last.pose.pose.position.z = rtk_to_baselink_translation.z();

            odom_last.pose.pose.orientation.x = rtk_to_baselink_q.x();
            odom_last.pose.pose.orientation.y = rtk_to_baselink_q.y();
            odom_last.pose.pose.orientation.z = rtk_to_baselink_q.z();
            odom_last.pose.pose.orientation.w = rtk_to_baselink_q.w();

            odom_last.header.frame_id = "odom";
            odom_last.header.stamp = ros::Time::now();
            
        }
        odom_pub.publish(odom_last);
        if(write_flag)
        {
