...
POINT_CLOUD_REGISTER_POINT_STRUCT (PointXYZRPYT,
(float, x, x) (float, y, y)
(float, z, z)
(float, roll, roll) (float, pitch, pitch) (float, yaw, yaw)
(double, time, time))
...
typedef PointXYZRPYT    PointType6D;
...
public:
    ...
    //added by wzf
    pcl::PointCloud<PointType6D>::Ptr keyFrameTransform;
    fileoperate f;
    ...
    void allocateMemory()
    {
        ...
        resetLIO();
        keyFrameTransform.reset(new pcl::PointCloud<PointType6D>());
        //added by gc
        for (int i = 0; i < 6; ++i){
...
#pragma omp parallel for num_threads(numberOfCores)
	...
	std::string loadSCDDirectory = "/home/nvidia/scliosam/data/Scans/";
       	std::string loadKeyFramePcdPath = "/home/nvidia/scliosam/data/transformations.pcd";
       	f.read_filelists( loadSCDDirectory, files_lists, "pcd" );
        f.sort_filelists( files_lists,"pcd");
	//std::cout<<"reading"<<std::endl;
	std::cout<<"*************************************************************"<<std::endl;
        pcl::PointCloud<pcl::PointXYZI>::Ptr cloud (new pcl::PointCloud<pcl::PointXYZI>);
        for(int i = 0; i < files_lists.size();i++)
        {
            std::cout <<files_lists.at(i)<<' ';
            // scManager.readSCDsFromFileAndSetSC(files_lists.at(i));
            //Open the PCD file
            if (pcl::io::loadPCDFile<pcl::PointXYZI> (loadSCDDirectory + files_lists.at(i), *cloud) == -1) 
            {
                PCL_ERROR ("Couldn't read keyFrame\n");
            }
            scManager.makeAndSaveScancontextAndKeys(*cloud);
        }

        if(pcl::io::loadPCDFile<PointType6D>(loadKeyFramePcdPath,*keyFrameTransform) == -1)
        {
            PCL_ERROR("Couldn't read transformation\n");
        }
        std::cout<<"x---------------->"<<keyFrameTransform->points[10].x<<std::endl;
        std::cout<<"y---------------->"<<keyFrameTransform->points[10].y<<std::endl;
        std::cout<<"z---------------->"<<keyFrameTransform->points[10].z<<std::endl;
        std::cout<<"r---------------->"<<keyFrameTransform->points[10].roll<<std::endl;
        std::cout<<"p---------------->"<<keyFrameTransform->points[10].pitch<<std::endl;
        std::cout<<"y---------------->"<<keyFrameTransform->points[10].yaw<<std::endl;
	...
	//cloudScanForInitialize->clear();
        ...
        auto scmatchresult = scManager.detectLoopClosureID();
        if(scmatchresult.second > 3.141592654)scmatchresult.second -= 6.283185307;
        else scmatchresult.second = scmatchresult.second;
        transformInTheWorld[2] = keyFrameTransform->points[scmatchresult.first].yaw - scmatchresult.second;
        transformInTheWorld[3] = keyFrameTransform->points[scmatchresult.first].x;
        transformInTheWorld[4] = keyFrameTransform->points[scmatchresult.first].y;
        //transformInTheWorld[5] = keyFrameTrajectory->points[scmatchresult.first].z;
        ...
        std::cout << "the rpypose before initializing is: x" << transformInTheWorld[0] << " y" << transformInTheWorld[1]
                  << " z" << transformInTheWorld[2] <<std::endl;
